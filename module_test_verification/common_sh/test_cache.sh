...
if [ -z "${REPO_ROOT:-}" ]; then
    ...
    while [ "$possible_repo_root" != "/" ]; do
        if [ -d "$possible_repo_root/module_test_verification/cache_files" ]; then
    ...
    if [ -z "${REPO_ROOT:-}" ]; then
        ...
        if [[ "$(basename "$sourcing_script")" == "test_cache.sh" ]] || [[ "$script_dir" == *"/module_test_verification/common_sh" ]]; then
...
CACHE_DIR="${REPO_ROOT}/module_test_verification/cache_files"
