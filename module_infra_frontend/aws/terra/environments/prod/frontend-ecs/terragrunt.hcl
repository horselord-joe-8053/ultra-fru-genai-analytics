# Frontend (ECS) layer for prod - S3 + CloudFront, depends on ECS for ALB DNS
include "root" {
...
dependency "app" {
  ...
  mock_outputs = {
    alb_dns_name = "alb-prod-ecs-placeholder.elb.us-east-1.amazonaws.com"
  }
...
inputs = {
  ...
  container_type        = "ecs"
  alb_dns_name          = try(dependency.app.outputs.alb_dns_name, "alb-prod-ecs-placeholder.elb.us-east-1.amazonaws.com")
