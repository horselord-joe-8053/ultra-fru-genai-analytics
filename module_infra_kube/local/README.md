...
./run_scripts/main_application_scripts/local/kube/setup.sh minikube

# Using kind
./run_scripts/main_application_scripts/local/kube/setup.sh kind

# Using Docker Desktop
./run_scripts/main_application_scripts/local/kube/setup.sh docker-desktop
```

### 2. Install NGINX Ingress Controller

```bash
./run_scripts/main_application_scripts/local/kube/install-ingress.sh minikube
```
...
- Same Kubernetes manifests (`module_infra_kube/shared/`) work identically on local and cloud
- Database can be run in Docker Compose (`module_infra_nonkube/local/docker-compose.yml`) or in Kubernetes
