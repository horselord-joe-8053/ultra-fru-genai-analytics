...
find_manifests_directory() {
    ...
    local possible_dirs=(
        "$repo_root/module_infra_kube/shared"
        "$repo_root/k8s"
    ...
    if [ -f "$configmap_template" ]; then
        ...
        if command_exists envsubst; then
            ...
            local possible_paths=(
                "${repo_root}/module_infra_basic/aws/environments/${env_infra}/infrastructure"
            ...
            local possible_eks_paths=(
                "${repo_root}/module_infra_kube/aws/environments/${environment}/eks"
            )
