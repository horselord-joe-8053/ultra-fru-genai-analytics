...
Called by run_analytics.py after computing analytics.
"""
import os
import json
import psycopg2
...
def save_analytics_to_db(
    ...
    Args:
        sales_by_brand: Brand-level analytics
        store_performance: Store performance metrics
        feedback_analysis: Feedback sentiment analysis
        top_models: Top-selling models
        price_stats: Price distribution statistics
        total_records: Total record count
        total_revenue: Total revenue
        db_config: Optional DB config (defaults to environment variables)
