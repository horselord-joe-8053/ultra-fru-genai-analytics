...
check_aws_credentials_from_env() {
    ...
    if [ -n "${AWS_ACCESS_KEY_ID:-}" ] || [ -n "${AWS_SECRET_ACCESS_KEY:-}" ]; then
        ...
        log_info "  Run: ./orchestration/aws/setup-aws-profiles.sh to set up profiles"
    fi
    ...
    if [ $missing_vars -eq 1 ]; then
        ...
        log_info "Run: ./orchestration/aws/setup-aws-profiles.sh after setting these variables"
    fi
