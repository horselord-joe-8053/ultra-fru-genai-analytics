...
fetch_terraform_outputs() {
    ...
    # From orchestration/aws/verification/ directory: go up 3 levels to repo root
    if [ -z "${REPO_ROOT:-}" ]; then
        local script_dir
    ...
    if [ "$container_type" = "ecs" ]; then
        if [ -f "$REPO_ROOT/module_infra_kubetypes/nonkube/aws/verification/fetch-deployment-info-ecs.sh" ]; then
            ...
            source "$REPO_ROOT/module_infra_kubetypes/nonkube/aws/verification/fetch-deployment-info-ecs.sh"
            fetch_ecs_deployment_info
        else
            log_warning "ECS deployment info script not found at: $REPO_ROOT/module_infra_kubetypes/nonkube/aws/verification/fetch-deployment-info-ecs.sh"
        fi
    elif [ "$container_type" = "eks" ]; then
        if [ -f "$REPO_ROOT/module_infra_kubetypes/kube/aws/verification/fetch-deployment-info-eks.sh" ]; then
            # shellcheck source=/dev/null
            source "$REPO_ROOT/module_infra_kubetypes/kube/aws/verification/fetch-deployment-info-eks.sh"
            fetch_eks_deployment_info
        else
            log_warning "EKS deployment info script not found at: $REPO_ROOT/module_infra_kubetypes/kube/aws/verification/fetch-deployment-info-eks.sh"
        fi
    else
