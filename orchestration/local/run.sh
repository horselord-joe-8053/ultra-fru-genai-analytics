...
main() {
    ...
    if ! "$REPO_ROOT/orchestration/prerequisites/check-and-install.sh" "local"; then
    ...
    if [ "$PREEMPT" = "true" ]; then
        ...
        local teardown_cmd="$REPO_ROOT/orchestration/local/teardown-resources-all.sh --container-type $CONTAINER_TYPE"
        if [ "${DRY_RUN:-false}" = "true" ]; then
    ...
    if [ "$CONTAINER_TYPE" = "kube" ]; then
        ...
        if ! "$REPO_ROOT/module_infra_kubetypes/kube/local/setup.sh" "$K8S_TYPE"; then
            elapsed=$(( $(date +%s) - step_start_time ))
        ...
        if ! "$REPO_ROOT/module_infra_kubetypes/kube/local/install-ingress.sh" "$K8S_TYPE"; then
            elapsed=$(( $(date +%s) - step_start_time ))
    ...
    if [ "$SKIP_FRONTEND" = false ]; then
        ...
        echo ""
        log_info "Or run: ./orchestration/run.sh local $CONTAINER_TYPE (with frontend)"
    fi
    perf_phase_end 5
