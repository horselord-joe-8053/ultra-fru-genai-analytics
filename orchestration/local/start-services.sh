...
DOCKER_DIR="$REPO_ROOT/module_infra_kubetypes/nonkube/local"
...
start_services() {
    ...
    if [ "$SKIP_CHECK" = false ] && [ "$FORCE_START" = false ]; then
        if check_services_running; then
            ...
            log_info "To apply .env changes, run: ./orchestration/local/start-services.sh --force"
            
            # Still wait for health checks to ensure they're ready
            load_env_file || true
            source "$REPO_ROOT/orchestration/common/wait-for-service.sh"
