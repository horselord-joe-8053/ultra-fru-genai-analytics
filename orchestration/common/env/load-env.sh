...
# Detect repo root based on this script's location (orchestration/common/env/ -> 3 levels up)
...
require_delta_lake_package() {
    ...
    if [ -z "${DELTA_LAKE_PACKAGE:-}" ]; then
        ...
        log_info ""
        log_info "You can run: ./orchestration/local/setup-env.sh to create/update .env file"
        return 1
    fi
...
#
# See: orchestration/common/env/load-image-identifiers.sh
