#!/bin/bash
# Wrapper script to load data into database
# Routes to provider-specific implementation (local, aws, azure, gcp)
# Usage: ./load_data.sh [provider] [additional-args...]
...
case "$PROVIDER" in
    local)
        source "$SCRIPT_DIR/load_data_local.sh"
        load_data_local "$@"
        ;;
    aws)
        source "$SCRIPT_DIR/load_data_aws.sh"
        load_data_aws "$@"
        ;;
    *)
