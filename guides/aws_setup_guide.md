...
# Container Image Prefix (will be replaced with ECR URI for AWS deployments)
# For local deployments, this can be a simple identifier
...
- Tag and push image to ECR (tag is auto-generated from git commit SHA)
- Export `CONTAINER_IMAGE` environment variable (dynamically generated from IMAGE_PREFIX:IMAGE_TAG)

### 7.3 Container Image Configuration

The container image is **automatically generated** from `IMAGE_PREFIX:IMAGE_TAG`:
- `IMAGE_PREFIX`: Set in `.env` (default: `fru-api-img-default-prefix`)
- `IMAGE_TAG`: Auto-generated from git commit (format: `fru-<env>-<date>-<sha>-<commit-slug>` or `fru-<env>-<date>-<sha>-dirty` for uncommitted changes)
  - **Format**: `fru-<environment>-<YYYYMMDD>-<git-sha>-<commit-message-slug>`
  - **Example (clean)**: `fru-dev-20260108-999a986-fix-teardown-script-path`
  - **Example (dirty)**: `fru-dev-20260108-999a986-dirty`
  - **Benefits**: Easy to search by date, environment, or commit message keywords
  - **Note**: Commit message slug is sanitized to only contain valid Docker tag characters (lowercase letters, digits, hyphens, underscores, periods)

For AWS deployments, `IMAGE_PREFIX` is automatically replaced with the actual ECR URI:
```
<account-id>.dkr.ecr.<region>.amazonaws.com/fru-api:fru-<env>-<date>-<sha>-<commit-slug>
```

**Note:** You do NOT need to set `CONTAINER_IMAGE` in `.env`. It is dynamically generated by the deployment scripts.
...
✅ TF_STATE_BUCKET
✅ IMAGE_PREFIX (default: fru-api-img-default-prefix)
```
