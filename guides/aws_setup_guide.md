...
- `IMAGE_TAG`: Auto-generated from git commit SHA (format: `git-<short-sha>` or `git-<short-sha>-dirty-<hash>` for uncommitted changes)

For AWS deployments, `IMAGE_PREFIX` is automatically replaced with the actual ECR URI:
```
<account-id>.dkr.ecr.<region>.amazonaws.com/fru-api:git-<sha>
```
