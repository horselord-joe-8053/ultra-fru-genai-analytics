...
deploy_frontend() {
    ...
    # Require Terraform output - fail if not available (unless dry-run: use placeholder)
    if [ -z "$s3_bucket_name" ]; then
        if [ "$DRY_RUN" = "true" ]; then
            s3_bucket_name="dry-run-bucket-placeholder"
            log_info "[DRY-RUN] No Terraform output (frontend layer not applied); using placeholder bucket for preview"
        else
    ...
    if [ "$DRY_RUN" = "true" ]; then
        ...
        if [ "$S3_BUCKET_NAME" = "dry-run-bucket-placeholder" ]; then
            log_info "[DRY-RUN] Placeholder bucket (Terraform not applied); would sync frontend after apply"
        elif [ ! -d "$REPO_ROOT/module_app_core/frontend/dist" ]; then
            log_info "[DRY-RUN] No frontend dist; would build and sync after apply"
        else
    ...
    if [ -d "$FRONTEND_TERRA_DIR" ] && command_exists terragrunt; then
        ORIG_DIR=$(pwd)
        cd "$FRONTEND_TERRA_DIR" 2>/dev/null || {
            log_warning "Could not access Terraform frontend directory for CloudFront outputs"
