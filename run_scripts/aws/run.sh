...
check_or_build_image() {
    ...
    # Note: IMAGE_TAG will be auto-generated by build-push-ecr.sh if not set
    # This ensures unique tags per code commit, allowing Terraform to detect changes
    
    # If IMAGE_TAG is explicitly set, check if that specific image exists
    if [ -n "${IMAGE_TAG:-}" ]; then
    ...
    # Image doesn't exist (or IMAGE_TAG not set), build and push it
    # Generate IMAGE_TAG here so we can use it after build-push-ecr.sh runs (in subshell)
    # This ensures Terraform gets the actual tag used (git SHA or provided tag)
    if [ -z "${IMAGE_TAG:-}" ]; then
    ...
    log_info "Note: Image tag will be auto-generated from git commit SHA if not specified"
    
    if "$SCRIPT_DIR/common_ecs_eks/build-push-ecr.sh"; then
        # Use the IMAGE_TAG we generated/exported above
        # This ensures Terraform gets the git SHA tag, not 'latest'
        export CONTAINER_IMAGE="$ECR_REPO_URI:$IMAGE_TAG"
