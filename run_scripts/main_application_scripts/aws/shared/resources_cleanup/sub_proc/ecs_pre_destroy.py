#!/usr/bin/env python3
"""
ECS pre-destroy: stop ECS services (scale to 0), empty ECS frontend and analytics S3 buckets.

Called by teardown-resources-all.sh before terraform destroy when --container-type ecs.
Uses subprocess to invoke stop-ecs-services.sh; uses boto3 to empty S3.
"""
...
def run_stop_ecs_services(
    ...
    stop_script = repo_root / "run_scripts/main_application_scripts/aws/ecs/helpers/stop-ecs-services.sh"
    logger_sh = repo_root / "orchestration/shared/logger.sh"
    load_env_sh = repo_root / "orchestration/shared/load-env.sh"
    if not stop_script.exists():
        print(f"  Warning: {stop_script} not found; skipping stop ECS services", file=sys.stderr)
        return True
    cmd = (
        f'set -e; source "{logger_sh}" 2>/dev/null || true; '
        f'source "{load_env_sh}" 2>/dev/null || true; '
        f'source "{stop_script}"; '
        f'stop_ecs_services "{cluster_name}" "{profile}" "{region}" "{str(dry_run).lower()}"'
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="ECS pre-destroy: stop services, empty S3 buckets.")
    ...
    buckets = [
        f"{PROJECT_NAME}-{args.environment}-frontend-ecs-{account_id}",
        f"{PROJECT_NAME}-{args.environment}-analytics-data-{account_id}",
    ]

    if not run_stop_ecs_services(REPO_ROOT, cluster_name, args.profile, args.region, args.dry_run):
        print("Warning: stop ECS services had issues (continuing)", file=sys.stderr)
