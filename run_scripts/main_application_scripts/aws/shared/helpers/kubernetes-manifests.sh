    ...
    if [ -f "$configmap_template" ]; then
        ...
        if command_exists envsubst; then
            ...
            local possible_eks_paths=(
                "${repo_root}/infra/terraform/providers/aws/environments/${environment}/eks"
                "${repo_root}/infra/terraform/environments/${environment}/eks"
            )
...
apply_kubernetes_manifests() {
    ...
    # Generate Deployment with CONTAINER_IMAGE (always regenerate if CONTAINER_IMAGE is available)
    # This ensures CONTAINER_IMAGE env var is set correctly, not just the image field
    local generated_dir="$manifests_dir/generated"
    local deployment_template="$manifests_dir/templates/deployment.template.yaml"
    local deployment_output="$generated_dir/deployment-generated.yaml"
    
    if [ -f "$deployment_template" ] && [ -n "$container_image" ]; then
        ...
        if [ -n "$envsubst_cmd" ]; then
            ...
            log_info "CONTAINER_IMAGE environment variable set to: $container_image"
        else
    ...
    elif [ -f "$deployment_template" ] && [ -z "$container_image" ]; then
        log_warning "CONTAINER_IMAGE not set - deployment will be generated without image substitution"
        log_warning "This may cause issues if the template requires CONTAINER_IMAGE"
    fi
