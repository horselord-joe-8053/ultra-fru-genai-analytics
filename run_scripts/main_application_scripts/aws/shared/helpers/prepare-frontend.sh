...
build_frontend_if_needed() {
    ...
    if [ ! -d "$repo_root/module_app_core/frontend/dist" ]; then
        ...
        local src_mtime="0"
        local src_files
        src_files=$(find "$repo_root/module_app_core/frontend/src" -type f \( -name "*.tsx" -o -name "*.ts" -o -name "*.jsx" -o -name "*.js" -o -name "*.css" \) 2>/dev/null | head -1)
        if [ -n "$src_files" ]; then
        ...
        if [ -f "$repo_root/module_app_core/frontend/package.json" ]; then
            package_mtime=$(stat -f "%m" "$repo_root/module_app_core/frontend/package.json" 2>/dev/null || stat -c "%Y" "$repo_root/module_app_core/frontend/package.json" 2>/dev/null || echo "0")
        fi
