...
check_service_status() {
    ...
    if [ "$container_type" = "ecs" ]; then
        if [ -f "$repo_root/run_scripts/main_application_scripts/aws/ecs/verification/check-service-status.sh" ]; then
            # shellcheck source=/dev/null
            source "$repo_root/run_scripts/main_application_scripts/aws/ecs/verification/check-service-status.sh"
            check_ecs_service_status "$environment"
        else
            log_warning "ECS service status script not found at: $repo_root/run_scripts/main_application_scripts/aws/ecs/verification/check-service-status.sh"
        fi
    fi
    
    # Check EKS pod status
    if [ "$container_type" = "eks" ]; then
        if [ -f "$repo_root/run_scripts/main_application_scripts/aws/eks/verification/check-service-status.sh" ]; then
            # shellcheck source=/dev/null
            source "$repo_root/run_scripts/main_application_scripts/aws/eks/verification/check-service-status.sh"
            check_eks_pod_status
        else
            log_warning "EKS service status script not found at: $repo_root/run_scripts/main_application_scripts/aws/eks/verification/check-service-status.sh"
        fi
    fi
