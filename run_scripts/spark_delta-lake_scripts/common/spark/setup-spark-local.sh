#!/bin/bash
# Setup Spark 4.0.1 environment locally (optional)
# Spark runs in Docker container, so local setup is only needed for:
#   - Running Spark jobs manually outside Docker
#   - Testing Spark scripts directly
#
# This setup is idempotent (safe to run multiple times)
...
setup_spark_local() {
    ...
    # Spark is not installed locally
    log_warning "Spark 4.0.1 is not installed locally"
    log_info ""
    log_info "Note: Spark runs in Docker container, so local setup is OPTIONAL"
    log_info ""
    log_info "Local Spark is only needed if you want to:"
    log_info "  - Run Spark jobs manually from host machine (outside Docker)"
    log_info "  - Test Spark scripts directly without Docker"
    log_info ""
    log_info "To install locally:"
    log_info "  1. Download Spark 4.0.1 from: https://spark.apache.org/downloads.html"
