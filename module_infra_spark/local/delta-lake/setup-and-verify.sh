...
if [ "$PREEMPT" = "true" ]; then
    ...
    teardown_cmd="$REPO_ROOT/module_infra_spark/common/delta-lake/teardown-delta.sh --local --environment $ENVIRONMENT"
...
if [ "$DRY_RUN" = "true" ]; then
    ...
    if ! "$REPO_ROOT/module_infra_spark/common/delta-lake/setup-delta-lake.sh"; then
...
if [ "$DRY_RUN" = "true" ]; then
    ...
    create_cmd="$REPO_ROOT/module_infra_spark/common/delta-lake/create-delta-table.sh $CSV_PATH $OUTPUT_PATH"
