...
| Path | Action |
|------|--------|
| **misc_scripts/** | Moved `misc_scripts/refresh-env-example.sh` → `scripts/refresh-env-example.sh`, removed `misc_scripts/`. |

## Removed

| Path | Action |
|------|--------|
| **infra/** | All references updated to module_* paths; directory removed. Docker → `module_infra_nonkube/local`, Terraform/k8s → `module_infra_basic`, `module_infra_kube`, `module_infra_nonkube`. |
...
- **run_scripts/main_application_scripts/aws/eks/** – aws/run.sh now calls `module_infra_kube/aws/deploy.sh`, but `run_scripts/.../aws/verification/*.sh` still source `run_scripts/.../aws/eks/verification/*.sh`. Update those to `module_infra_kube/aws/verification/` before removing.
...
- **Root `./run.sh`** → `orchestration/run.sh` → **exec’s** `run_scripts/main_application_scripts/local/run.sh` or `run_scripts/main_application_scripts/aws/run.sh`. Those scripts contain the flow (local vs AWS, kube vs nonkube, terraform, start-services, etc.) and **call into** module_* (e.g. `module_infra_kube/aws/deploy.sh`, `module_infra_db/...`, `module_infra_nonkube/local/...`). The module_* dirs are **invoked by** run_scripts; they do not replace them.
