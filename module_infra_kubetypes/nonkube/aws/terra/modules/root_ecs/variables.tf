...
variable "desired_count" {
  type        = number
  description = "Desired number of tasks (legacy name)"
  default     = 1
}
...
variable "container_type" {
  type        = string
  description = "Container type (ecs or eks) - used by analytics scheduler to determine if S3A packages are needed and to create separate frontend instances"
  default     = "ecs"
}
